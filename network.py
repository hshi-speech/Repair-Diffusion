...
class NCSNpp(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        # Input layer
        self.input_layer = conv3x3(channels, nf)
        hs_c = [nf]
        in_ch = nf
        ...
        # -------> [256, t] to [128, t]
        # in_ch = nf = 128
        self.shared_encoder_p_1 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(down=True, down_f=True, in_ch=in_ch)
        )

        # -------> shared encoder [2]: 2x128-128 + down
        # -------> [128, t] to [64, t]
        self.shared_encoder_p_2 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(down=True, down_f=True, in_ch=in_ch)
        )
        ...
        # -------> [64, t] to [32, t]
        self.shared_encoder_p_3 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(down=True, down_f=True, in_ch=in_ch)
        )
        ...
        # -------> [32, t] to [32, t/2]
        self.score_encoder_p_1 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
        ...
        # -------> [32, t/2] to [16, t/4]
        self.score_encoder_p_2 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
        ...
        # -------> [16, t/4] to [8, t/8]
        self.score_encoder_p_3 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(down=True, in_ch=in_ch)
        )
        ...
        # -------> [8, t/8] to [8, t/16]
        self.score_encoder_p_4 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
        ...
        # -------> [8, t/16] to [4, t/32]
        self.score_encoder_p_5 = nn.Sequential(
            ResnetBlock(in_ch=in_ch, out_ch=in_ch),
            ResnetBlock(down=True, in_ch=in_ch)
        )
    ...
    def forward(self, x, time_cond, w_training=0):
        ...
        # ----> first shared layer
        h = self.shared_encoder_p_1[0](hs[-1], temb)
        hs.append(h)
        hs_deter.append(h)
        h = self.shared_encoder_p_1[1](h, temb)
        hs.append(h)
        hs_deter.append(h)
        h = self.shared_encoder_p_1[2](h, temb)
        hs.append(h)
        hs_deter.append(h)

        # ----> second shared layer
        h = self.shared_encoder_p_2[0](h, temb)
        hs.append(h)
        hs_deter.append(h)
        h = self.shared_encoder_p_2[1](h, temb)
        hs.append(h)
        hs_deter.append(h)
        h = self.shared_encoder_p_2[2](h, temb)
        hs.append(h)
        hs_deter.append(h)

        # ----> third shared layer
        h = self.shared_encoder_p_3[0](h, temb)
        hs.append(h)
        hs_deter.append(h)
        h = self.shared_encoder_p_3[1](h, temb)
        hs.append(h)
        hs_deter.append(h)
        ...
        # ================================================= #
        h_deter = self.deter_embedding[0](h)
        hs.append(h_deter)
        ...
        h = self.score_encoder_p_1[0](h, temb)
        hs.append(h)
        h = self.score_encoder_p_1[1](h, temb)
        hs.append(h)

        # ----> second *score-encoder* layer
        h = self.score_encoder_p_2[0](h, temb)
        hs.append(h)
        h = self.score_encoder_p_2[1](h, temb)
        hs.append(h)

        # ----> third *score-encoder* layer
        h = self.score_encoder_p_3[0](h, temb)
        hs.append(h)
        h = self.score_encoder_p_3[1](h, temb)
        hs.append(h)

        # ----> forth *score-encoder* layer
        h = self.score_encoder_p_4[0](h, temb)
        h = self.score_encoder_p_4[1](h)
        hs.append(h)
        h = self.score_encoder_p_4[2](h, temb)
        hs.append(h)

        # ----> fifth *score-encoder* layer
        h = self.score_encoder_p_5[0](h, temb)
        hs.append(h)
        h = self.score_encoder_p_5[1](h, temb)
        hs.append(h)
        ...
        h = self.score_embedding[0](h, temb)
        ...
        # ================ Score Decoder ================== #
        # ================================================= #
        h = self.score_decoder_p_8[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_8[1](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_7[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_7[1](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_6[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_6[1](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_5[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_5[1](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_4[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_4[1](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_3[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_3[1](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_3[2](torch.cat([h, hs.pop()], dim=1), temb)


        h = self.score_decoder_p_2[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_2[1](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_2[2](torch.cat([h, hs.pop()], dim=1), temb)

        h = self.score_decoder_p_1[0](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_1[1](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_1[2](torch.cat([h, hs.pop()], dim=1), temb)
        h = self.score_decoder_p_1[3](torch.cat([h, hs.pop()], dim=1), temb)
